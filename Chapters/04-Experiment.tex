...
Selection criteria: explain why these benchmarks were chosen (e.g., standard CEC functions, classic test functions: Sphere, Rastrigin, Ackley, Schwefel, Rosenbrock, Griewank, etc.)
...
Problem properties: dimensionality, known optimum, search domain.


\section{Benchmark Problems}

To ensure a comprehensive and fair evaluation, the algorithms are tested on a set of well-established continuous benchmark functions commonly used in the swarm intelligence and evolutionary computation literature. The selected functions cover a wide range of characteristics:

\begin{itemize}
    \item \textbf{Unimodal:} Sphere, Schwefel 2.22.
    \item \textbf{Multimodal:} Rastrigin, Ackley, Griewank.
    \item \textbf{Deceptive or Rugged:} Schwefel 2.26, Michalewicz, Rosenbrock.
\end{itemize}

Each function is tested in multiple dimensions (\(d = 100, 500, 1000\)), with search bounds and known optima defined according to standard references. This setup ensures that the proposed algorithms are challenged on diverse topological landscapes ranging from smooth convex to rugged multi-peaked spaces.
...
Your new variants (Contrarian, Defeatist, Eschewer, Escapist, all hybrids)

Standard PSO

Other state-of-the-art baselines (if any)

Parameter settings: inertia weight, acceleration coefficients, role fractions/probabilities — justify choices (tuned vs. default).

Environment: hardware specs, software libraries, random seed policy.

Performance metrics: e.g., best fitness, mean fitness, standard deviation, convergence curves, runtime.

Repetitions: number of independent runs per function and dimension.
...
Tables of average best fitness, standard deviation, rank ordering.

5.2 Convergence Plots

Representative plots comparing convergence trajectories.

5.3 Statistical Tests

Formal statistical significance tests (e.g., Wilcoxon, Friedman, ANOVA) with p-values and post-hoc ranking.

5.4 Sensitivity Analysis

Explore effect of varying role fractions/probabilities.

5.5 Scalability Analysis

Results for increasing dimensionality or runtime constraints.

5.6 Ablation Study

Optional but valuable: isolate each role or hybrid component to quantify its isolated contribution.

\section{Discussion}

Interpret key findings:

When does your method outperform standard PSO?
